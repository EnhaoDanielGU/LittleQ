...
def handle_bar(counter,  # a counter for number of minute bars that have already been tested
            ...
            if(memory.buy_sell[int(i/2)]and memory.timer[int(i/2)]>0 and memory.timer[int(i/2)]<=limit):
                ...
                if(cash_balance>20000):
                    if(position_new[asset_index[int(i/2)]]>=0):
                        ...
                        position_new[asset_index[int(i/2)]]=-position_new[asset_index[int(i/2)]]+(cash_balance-20000)/2/memory.dataframe_collection[i+1].iloc[counter]['average']
                else:
                    if(position_new[asset_index[int(i/2)]]< 0):
    ...
    '''Last week, we try reverse MACD strategy but it does not work, we still lose money, so we use the ordinary MACD. And we don't have much time to
    modify the strategy as we are occupied by other issues. So, we just do little modification this week.'''
    # End of strategy
