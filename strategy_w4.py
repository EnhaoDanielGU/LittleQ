...
short_window = 12  # the short-window period, set to be 12 minute
extra_memry = 9  # the number of extra data we will record in memory, this is the span of exponential weighted moving average of the difference of short minus long
...
def handle_bar(counter,  # a counter for number of minute bars that have already been tested
    ...
    if (counter >= memry_length):
        ...
        for i in [0,2,4,6]:
            ...
            memory.dataframe_collection[i+1]['Nine']=memory.dataframe_collection[i+1].MACD.ewm(span=9).mean()
            ...
            if(memory.buy_sell[int(i/2)]and memory.timer[int(i/2)]>0 and memory.timer[int(i/2)]<=limit):
                #we set our cash buffer to be 15000, whenever our cash is lower than 15000, we only do position clearing
                if(cash_balance>30000):
                    if(position_new[asset_index[int(i/2)]]>=0):
                        position_new[asset_index[int(i/2)]] =-position_new[asset_index[int(i/2)]]-(cash_balance-15000)/2/memory.dataframe_collection[i+1].iloc[counter]['average']
                    else:
                        position_new[asset_index[int(i/2)]]-=(cash_balance-30000)/2/memory.dataframe_collection[i+1].iloc[counter]['average']
                else:
                    if(position_new[asset_index[int(i/2)]]>0):
    ...
    '''super-important remark: when conducting the backtesting process, we found that our strategy stably lose money, so we decide to use reserse-MACD strategy, so the original bullish signal
    is now bearish signal and the orginal bearish signal is now bullish. Please interpret all the buys above as sell and sells as buy'''
    # End of strategy
